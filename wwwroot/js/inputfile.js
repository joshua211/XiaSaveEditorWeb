(function () {
  window.BlazorInputFile = {
    init: function init(elem, componentInstance) {
      ...
      elem.addEventListener("change", function handleInputFileChange(event) {
        ...
        componentInstance.invokeMethodAsync("NotifyChange", fileList).then(
          function () {
      ...
      return new Promise(function (resolve) {
        ...
        .then(function (loadedImage) {
          return new Promise(function (resolve) {
            ...
            var desiredHeightRatio = Math.min(
              1,
              maxHeight / loadedImage.height
            );
            var chosenSizeRatio = Math.min(
              desiredWidthRatio,
              desiredHeightRatio
            );
            ...
            canvas
              .getContext("2d")
              .drawImage(loadedImage, 0, 0, canvas.width, canvas.height);
    ...
    ensureArrayBufferReadyForSharedMemoryInterop: function ensureArrayBufferReadyForSharedMemoryInterop(
      elem,
      fileId
    ) {
      return getArrayBufferFromFileAsync(elem, fileId).then(function (
        arrayBuffer
      ) {
    ...
    readFileDataSharedMemory: function readFileDataSharedMemory(readRequest) {
      ...
      var inputFileElementReferenceId = Blazor.platform.readStringField(
        readRequest,
        0
      );
      var inputFileElement = document.querySelector(
        "[_bl_" + inputFileElementReferenceId + "]"
      );
      ...
      var bytesToRead = Math.min(
        maxBytes,
        sourceArrayBuffer.byteLength - sourceOffset
      );
      var sourceUint8Array = new Uint8Array(
        sourceArrayBuffer,
        sourceOffset,
        bytesToRead
      );
  ...
  function getFileById(elem, fileId) {
    ...
    if (!file) {
      throw new Error(
        "There is no file with ID " +
          fileId +
          ". The file list may have changed"
      );
    }
